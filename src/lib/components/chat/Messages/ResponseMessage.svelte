<script lang="ts">
	...
	// copyLink: copy a direct link to this message
	const copyLink = async () => {
		const url = `${window.location.origin}/c/${chatId}?message=${message.id}`;
		const res = await _copyToClipboard(url, false);
		if (res) {
				...
				{#if message.timestamp}
					...
					>
						<Tooltip content={dayjs(message.timestamp * 1000).format('LLLL')}>
							<!-- TODO maybe format this via browser's/user's locale? -->
					...
					>
						{#if message.done || siblings.length > 1}
							...
							{#if message.done}
								...
								<Tooltip content={$i18n.t('Copy Link')} placement="bottom">
									<button
										class="{isLastMessage
											? 'visible'
											: 'invisible group-hover:visible'} p-1.5 hover:bg-black/5 dark:hover:bg-white/5 rounded-lg dark:hover:text-white hover:text-black transition"
