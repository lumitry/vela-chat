...
def upgrade() -> None:
    ...
    for row in rows:
        ...
        for mid, msg in history_messages.items():
            ...
            for f in rag_files:
                ftype = f.get('type') or 'file'
                file_id = f.get('id') if ftype not in ['collection', 'web_search'] else None
                url = f.get('url') if ftype not in ['collection', 'web_search'] else None
                ...
                if ftype == "collection":
                    ...
                    if "data" in f and isinstance(f["data"], dict):
                        data_copy = {k: v for k, v in f["data"].items() if k != "file_ids"}
                        if data_copy:
    ...
    print(f"Migration complete: {migrated_count} messages migrated, {files_migrated} files migrated")
